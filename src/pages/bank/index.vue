...
const headers = computed(() => {
	return [
		...
		{ title: t("bankCode"), key: "bankCode", align: "start" },
		{ title: t("bankName"), key: "bankName", align: "start" },
...
const getList = () => {
	loading.value = true;
	BankService.GetList(pagination.value)
		.then((res) => {
		...
		<WTable v-model:pagination="pagination" :loading="loading" :headers="headers" :items="List" :items-length="totalItems" @update:options="getList">
			<template v-slot:item.actions="{ item }">
				<div class="d-flex align-center">
					...
					<DeleteBtn :id="item.id" :action="BankService.Delete" @refresh="getList" />
				</div>
			</template>
		</WTable>
	</UiParentCard>
