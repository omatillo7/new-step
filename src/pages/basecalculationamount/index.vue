...
import BasecalculationamountService from "@/services/basecalculationamount";
...
const headers = computed(() => {
	return [
		...
		{ title: t("onDate"), key: "onDate", align: "start" },
...
const getList = () => {
	loading.value = true;
	BasecalculationamountService.GetList(pagination.value)
		.then((res) => {
	...
	<UiParentCard>
		<template #action>
			<v-row class="pt-2">
				...
				<v-col cols="12" lg="8" md="6" class="text-right" v-if="can('basecalculationamountEdit')">
					<v-btn color="primary" :to="{ name: 'basecalculationamountEdit', params: { id: 0 } }" class="ml-auto">
